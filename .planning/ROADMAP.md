...
- [ ] **Phase 0: Foundation & Safety Harness** - Build and prove the non-bypassable mutation gate, config/credential store, and startup capability probe before any AD write exists.
- [ ] **Phase 1: AD Query & Reporting (read-only)** - Launch the TUI, search/view scoped objects, and render correct read-only reports (console/CSV/HTML) that prove AD semantics before writes consume them.
- [ ] **Phase 2: Single-Object Lifecycle (writes begin)** - AD/local user, computer, and group-membership lifecycle for one object at a time, every change routed through the gate with truthful preview, scaled confirmation, and audit.
...
**Plans**: TBD (suggested 5-plan split below)
**UI hint**: no

Suggested plan split (heavy phase — 17 requirements, refined during `/gsd-plan-phase 0`):
- [ ] 00-01: Module scaffold (`adman.psd1/.psm1`, Public/Private loader, explicit `FunctionsToExport`), PSFramework config+logging backbone, `$ErrorActionPreference='Stop'` convention, `-Server`-pinning helper.
- [ ] 00-02: Non-secret config schema + load/validate/save/reload (fail closed on empty managed-OU / failed deny-list); gitignored `.store/`; startup capability probe (`Test-AdmanCapability`) feeding session flags (MENU-05, CONF-01/02/03/05).
- [ ] 00-03: Credential decision — pass-through by default, prompt only when rights insufficient; opt-in DPAPI credential file with re-prompt-on-restore (CONF-04/06).
- [ ] 00-04: Safety core — `Invoke-AdmanMutation` (THE GATE), `Test-AdmanTargetAllowed` (scope + deny-list + runtime-SID protection), `Confirm-AdmanAction` (ShouldProcess + typed bulk confirm), `Assert-AdmanBulkPolicy`; identical preview/execute resolution (SAFE-01/02/05/06/07/08/09/10).
- [ ] 00-05: Fail-closed append-only audit (`Write-AdmanAudit`), Recycle-Bin/recovery-posture preflight, and the Pester+lint guard proving no exported writer bypasses the gate (SAFE-03/04/08 exit gate).
...
**Total:** 6 phases, 23 plans (suggested), 58/58 v1 requirements mapped.
