...
must_haves:
  truths:
    ...
    - tests/Docs.Coverage.Tests.ps1 accesses the private Get-AdmanMenuDefinition through module scope after importing adman.psd1 and verifies menu/PromptSpec coverage and exported-function parameter coverage deterministically (D-02)
...
<task type="auto" tdd="true">
  ...
  <acceptance_criteria>
    ...
    - tests/Docs.Coverage.Tests.ps1 iterates each exported function from adman.psd1 FunctionsToExport, locates that function's section in docs/USAGE.md, and asserts that every non-common parameter name returned by Get-Command appears in the section text before the next `## ` heading or before a fenced code block. Common parameters excluded: Verbose, Debug, ErrorAction, WarningAction, InformationAction, ErrorVariable, WarningVariable, InformationVariable, OutVariable, OutBuffer, PipelineVariable, WhatIf, Confirm
