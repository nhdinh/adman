...
- **D-07: Self-tuning grace buffer = `max(14, msDS-LogonTimeSyncInterval) + 1 day`.**
  - **Preflight read (RPT-07 territory):** at `Initialize-Adman` time, read `ms-DS-Logon-Time-Sync-Interval` from the domain NC head (`CN=Directory Service,CN=Windows NT,CN=Services,CN=Configuration,...`) and cache it on `$script:Config`. On read failure, fall back to 14 (the AD default).
...
- **D-08: Recovery-posture preflight (RPT-07) at startup.** The existing `Private/Foundation/Get-AdmanRecoveryPosture.ps1` (Phase 0) is invoked during `Initialize-Adman` and its result (Recycle Bin enabled/disabled, Forest Functional Level, `ms-DS-Logon-Time-Sync-Interval`) is surfaced in the startup capability banner AND cached for the stale report. RPT-07 is a *report the preflight produces*, not a separate menu item — but a `Get-AdmanRecoveryPostureReport` Public verb is also exposed so a senior can call it directly (MENU-04).
