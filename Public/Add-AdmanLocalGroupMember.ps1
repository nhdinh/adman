...
function Add-AdmanLocalGroupMember {
    <#
    .SYNOPSIS
        Add-AdmanLocalGroupMember - add a local user to a local group through the local
        mutation gate (LUSR-02, D-02).

    .DESCRIPTION
        Thin prompt-and-dispatch Public verb. Builds the parameter hashtable and calls
        Invoke-AdmanLocalMutation -Verb 'Add-LocalGroupMember'. The local gate's policy
    ...
    .PARAMETER Name
        The local user name to add to the group.
    ...
    .EXAMPLE
        Add-AdmanLocalGroupMember -Name 'luser-fake' -Group 'Administrators'

    .EXAMPLE
        Add-AdmanLocalGroupMember -Name 'luser-fake' -Group 'Remote Desktop Users' -WhatIf
    #>
