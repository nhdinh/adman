...
function Remove-AdmanGroupMember {
    ...
    .PARAMETER Identity
        The user or computer to remove from the group. Accepts sAMAccountName, DN, GUID, or UPN.

    .PARAMETER GroupIdentity
        The AD group to remove the member from. Accepts name, DN, GUID, or SID.

    .PARAMETER Force
        Skip the per-verb confirmation prompt.

    .EXAMPLE
        Remove-AdmanGroupMember -Identity 'jdoe-fake' -GroupIdentity 'Helpdesk'

    .EXAMPLE
        Remove-AdmanGroupMember -Identity 'jdoe-fake' -GroupIdentity 'Domain Admins' -WhatIf
    #>
