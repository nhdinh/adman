...
<#
.SYNOPSIS
    GRP-02 contract tests for Remove-AdmanGroupMember.

.DESCRIPTION
    Pins the contract for the Remove group membership Public verb:
      * Test 2: calls Invoke-AdmanMutation with -Verb 'Remove-ADGroupMember',
      ...
      * Test 8 (HIGH #2 review fix): adman.psd1 FunctionsToExport contains
        'Remove-AdmanGroupMember' explicitly; Get-Command -Module adman resolves
...
Describe 'Remove-AdmanGroupMember: gate routing (GRP-02, D-04)' -Tag 'Unit' {

    It 'Test 2: calls Invoke-AdmanMutation with -Verb Remove-ADGroupMember, -Targets @($Identity), and $Parameters containing GroupIdentity' {
        ...
        Should -Invoke -ModuleName adman Invoke-AdmanMutation -Times 1 -ParameterFilter {
            $Verb -eq 'Remove-ADGroupMember' -and
    ...
    It 'Test 7: forwards -Force to the gate' {
        ...
        Remove-AdmanGroupMember -Identity 'jdoe' -GroupIdentity 'Helpdesk' -Force

        Should -Invoke -ModuleName adman Invoke-AdmanMutation -Times 1 -ParameterFilter {
            $Verb -eq 'Remove-ADGroupMember' -and $Force -eq $true
        }
    }
}

Describe 'Remove-AdmanGroupMember: manifest export (HIGH #2 review fix)' -Tag 'Unit' {

    It 'Test 8: adman.psd1 FunctionsToExport contains Remove-AdmanGroupMember explicitly; Get-Command resolves it' {
        $content = Get-Content $script:ManifestPath -Raw
        $content | Should -Match 'Remove-AdmanGroupMember'
        (Get-Command -Module adman -Name 'Remove-AdmanGroupMember' -ErrorAction SilentlyContinue) | Should -Not -BeNullOrEmpty
    }
}
